...
namespace QTRLib {
    namespace DrawDebug {
        ...
        // Ideally you would do something a bit more clever, but with what little rendering we do storing a lazy cache
        // like this is fine Depth
        typedef struct DSStateKey {
            ...
            size_t Hash() const {
                return std::hash<bool>()(write) ^ std::hash<bool>()(test) ^ std::hash<uint16_t>()(mode);
            }
        ...
        void SetBlendState(D3DContext& ctx, bool enable, D3D11_BLEND_OP blendOp = D3D11_BLEND_OP::D3D11_BLEND_OP_ADD,
                           ...
                           D3D11_BLEND src = D3D11_BLEND::D3D11_BLEND_ONE,
                           D3D11_BLEND dest = D3D11_BLEND::D3D11_BLEND_ZERO,
        ...
        public:
            Shader(const ShaderCreateInfo& createInfo, D3DContext& ctx) noexcept
                ...
                if (stage == PipelineStage::Vertex) {
                    auto result = context.device->CreateVertexShader(binary->GetBufferPointer(),
                                                                     binary->GetBufferSize(), nullptr, &program.vertex);
...
PS_OUTPUT main(PS_INPUT input) {
	...
	return output;
        ...
        typedef struct DrawnPoint {
            ...
            DrawnPoint(Point&& a_point, float& a_duration) : point(a_point), duration(a_duration) {};
        } DrawnPoint;
        ...
        private:
            ...
            void DrawBatch(uint32_t bufferIndex, LineList::const_iterator& begin, LineList::const_iterator& end) {
                ...
                uint32_t index = 0;
                auto buf =
                    reinterpret_cast<glm::vec4*>(vbo[bufferIndex]->Map(D3D11_MAP::D3D11_MAP_WRITE_DISCARD).pData);
                ...
                while (begin != end) {
                    ...
                    RE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, a_end, _end.x,
                                                     _end.y, _end.z, 1e-5f);
                    _end -= 0.5f;
                    _end *= 2.f;
        ...
        public:
            static void Install() {
                ...
                _Nullsub = trampoline.write_call<5>(
                    REL::RelocationID(35565, 36564).address() + REL::Relocate(0x748, 0xc2b), Nullsub);
            }
        ...
        public:
            ...
            static void DrawLine(const RE::NiPoint3& a_start, const RE::NiPoint3& a_end, glm::vec4 a_color) {
                glm::vec3 start{a_start.x, a_start.y, a_start.z};
